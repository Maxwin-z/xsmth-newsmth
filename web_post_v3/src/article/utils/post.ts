...
export function retrieveGroupPosts(html: string, page: number): IGroup {
  ...
  div.innerHTML = html;
  const title = (
    div.querySelector(".b-head .n-left") as HTMLSpanElement
  ).innerText.replace("文章主题: ", "");
  ...
  const posts = [].slice
    ...
    .map((table: HTMLTableElement) => {
      const author = (
        table.querySelector(".a-head .a-u-name") as HTMLAnchorElement
      ).innerText;
      const pid = parseInt(
        (
          table.querySelector("a.a-post,.a-func-forward") as HTMLAnchorElement
        ).href
          .split("/")
          .pop()
          ?.replace(/[^\d]/g, "") || "0",
...
*/
export function formatPost(body: string): {
  date: number;
  dateString: string;
  nick: string;
  content: string;
  ...
  const dateRegex =
    /^发信人:.+?<br ?\/?> 标.+?<br ?\/?> 发信站:.+?\([A-Z][a-z]{2} ([A-Z][a-z]{2}( |&nbsp;&nbsp;)\d+ \d{1,2}:\d{1,2}:\d{1,2} +\d{4})\)/;
...
function formatLikes(dom: HTMLElement): Array<ILike> {
  ...
  const likes = [].slice
    .call(dom.querySelectorAll("ul li"))
    .map((li: HTMLLIElement): ILike => {
