...
import { unloaded, getPost, savePost, removePost } from "../../jsapi";
  ...
  async (dispatch, getState) => {
    ...
    group.pages = group.pages.map((page) => {
      ...
      page.posts.forEach((post) => {
        ...
        if (dom) {
          domHeights[post.floor] = Math.ceil(
    ...
    await savePost(storageKey(post), instance);
    if (andUnload) unloaded();
  };

export const cacheInstance = async (post: IMainPost) => {
  try {
    let data: RootState = await getPost(storageKey(post));
...
export const removeInstance = async (post: IMainPost) => {
  await removePost(storageKey(post));
};
